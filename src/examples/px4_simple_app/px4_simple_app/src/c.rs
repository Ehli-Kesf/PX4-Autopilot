...
impl Metadata {
    ...
    #[doc(hidden)]
    pub const fn _unsafe_new(
        o_name: *const u8,
        o_size: u16,
        o_size_no_padding: u16,
        message_hash: u32,
        o_id: u16,
        o_queue: u8,
    ) -> Metadata {
    ...
    pub fn name(&self) -> &str {
        unsafe { core::str::from_utf8_unchecked(self.name_cstr().to_bytes()) }
    }

    /// Same as `name`, but as zero-terminated C string.
    pub fn name_cstr(&self) -> &CStr {
        unsafe { CStr::from_ptr(self.o_name as *const _) }
    }
    ...
    ////     unsafe { core::str::from_utf8_unchecked(self.fields_cstr().to_bytes()) }
    //// }
